...
void game::takeInput()
{
    while(SDL_PollEvent(&lTexture.event) != 0)
    {
        ...
        else if (lTexture.event.type == SDL_MOUSEBUTTONDOWN || (lTexture.event.type == SDL_KEYDOWN &&
		(lTexture.event.key.keysym.sym == SDLK_SPACE || lTexture.event.key.keysym.sym == SDLK_UP) && lTexture.event.key.repeat == 0) )
        {
...
bool game::checkReplay()
{
	...
	int x, y;
	SDL_GetMouseState(&x, &y);
	if (x > (lTexture.SCREEN_WIDTH - 100)/2 && x < (lTexture.SCREEN_WIDTH + 100) / 2 && y > 380 && y < 380 + 60)
	{
		return true;
	}
	return false;
